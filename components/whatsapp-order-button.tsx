...
export function WhatsAppOrderButton({ phoneNumber, className }: WhatsAppOrderButtonProps) {
  const { t } = useLanguage()
  ...
  return (
    <Button
      onClick={() => window.open(createOrderTemplate(), "_blank")}
    ...
    >
      ...
      {mounted ? t("whatsapp.orderViaWhatsApp") : "Order via WhatsApp"}
    </Button>
  )
}
